...
export function generateAskResponse(
  question: string,
  channelId: number,
  author: string,
  characterId: number
): (c: Client) => Promise<any> {
...
export function generateCharacters(channelId: number): (c: Client) => Promise<any> {
  return async (client: Client) => {
    const bot = new OpenAIApi(new Configuration({
      apiKey: process.env.OPENAI_API_KEY!
    }));
